  ...
  const hardware_interface::HardwareInfo & info)
{
  ...
  for (const hardware_interface::ComponentInfo & joint : info_.joints)
  {
      ...
      joint.parameters.count("gear_ratio") != 0)
    {
      ...
      torque_constants_.emplace_back(std::stof(joint.parameters.at("kt")));
        ...
        joint.parameters.count("vel_limit") != 0)
      {
        ...
        limits.first = std::stof(joint.parameters.at("vel_limit")) / 10.0 * erpm_conversion;
        limits.second = std::stof(joint.parameters.at("acc_limit")) / 10.0 * erpm_conversion;
        if (limits.first >= 32767 || limits.first <= 0)
        {
          RCLCPP_ERROR(
            rclcpp::get_logger("CubeMarsSystemHardware"),
            "velocity limit is not in range 0-32767: %d for can_id %s", limits.first, joint.name.c_str());
          return hardware_interface::CallbackReturn::ERROR;
        }
        else{
          RCLCPP_INFO(
            rclcpp::get_logger("CubeMarsSystemHardware"),
            "velocity limit for joint %s: %d", joint.name.c_str(), limits.first);
        }
        if (limits.second >= 32767 || limits.second <= 0)
        {
          RCLCPP_ERROR(
            rclcpp::get_logger("CubeMarsSystemHardware"),
            "acceleration limit is not in range 0-32767: %d for can_id %s", limits.second, joint.name.c_str());
          return hardware_interface::CallbackReturn::ERROR;
        }
        else{
          RCLCPP_INFO(
            rclcpp::get_logger("CubeMarsSystemHardware"),
            "acceleration limit for joint %s: %d", joint.name.c_str(), limits.second);
        }
